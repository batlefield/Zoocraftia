...
public class ZooConfig
{
    ...
    /**
     * Main constructor for the config, creates new file for the config
     * @param zp Plugin that is creating the config
     * @param s	 Name for the config
     */
    public ZooConfig(IZoocraftiaPlugin zp, String s)
    {
    	this(new File(Minecraft.getMinecraftDir(), "Zoo/" + zp.getPluginName() + s));
    }
    
    private ZooConfig(File file)
    {
        this.file = file;
    ...
    public Property getOrCreateBlockIdProperty(String key, int defaultId)
    {
        ...
        {
            Property property = getOrCreateIntProperty(key, ZooConfig.CATEGORY_BLOCK, defaultId);
